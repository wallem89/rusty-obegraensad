...
The default workspace members are `core` and `boards/pico`, so running Cargo from the repository root builds the portable core and the Raspberry Pi Pico firmware:
```
cargo build --release
```

To build the alternative member set consisting of `core` and `boards/esp32-s3-devkit-c-1`, install the ESP Rust and GCC toolchains once:
```
cargo install espup
espup install
```

Before building, load the environment generated by `espup` into each new shell. This puts the `xtensa-esp32s3-elf-gcc` linker on `PATH`:
```
source "$HOME/export-esp.sh"
```

Then use the ESP Rust toolchain and explicitly select the ESP32-S3 target:
```
cargo +esp build --release \
  --package obegraensad-core \
  --package obegraensad-esp32-s3-devkit-c-1 \
  --target xtensa-esp32s3-none-elf \
  -Z build-std=core
```

Because the ESP32-S3 firmware depends on `core`, the shorter equivalent is to build from its crate directory as shown in the ESP32-S3 section below.
...
```
For release builds and flashing, ensure the ESP Xtensa GCC toolchain is on `PATH`; with `espup`, run `source "$HOME/export-esp.sh"` in the current shell before running Cargo.
